...
class SpotCrime(ServiceBase):
    ...
    def checkcrime(lat, lon, radius):
        get_url = "https://api.spotcrime.com/crimes.json?lat=%s&lon=%s&radius=%s&key=%s" % (
            lat, lon, radius, "privatekeyforspotcrimepublicusers-commercialuse-877.410.1607")
        ...
        if json_response.get('crimes', None):
